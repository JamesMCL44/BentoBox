...
public class WorldFileData
{
    ...
    /**
     * Use this static method to create a new instance of this class. If null is returned, there was a problem so
     * any process relying on this should be cancelled.
     * Defaults to "REGION" if "type" is "WorldFileDataType.ALL"
     * @param world world
     * @param notifyPlayer player to notify of status
     * @param type world file data type
     * @param silent true or false
     * @return WorldFileData instance or null if there was a problem
     */
    ...
    {
        BentoBox.getInstance().log("[WorldData] " + text);
    ...
    private void testImage(CoordXZ region, List<Boolean> data) {
        ...
        BentoBox.getInstance().log(f.getAbsolutePath());
        try {
            // png is an image format (like gif or jpg)
            ImageIO.write(bi, "png", f);
        } catch (IOException ex) {
            BentoBox.getInstance().log("[SEVERE]" + ex.getLocalizedMessage());
        }
    }
}
