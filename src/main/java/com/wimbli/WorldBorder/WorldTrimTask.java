...
public class WorldTrimTask implements Runnable
{
    ...
    private void reportProgress()
    {
        ...
        sendMessage("[" + type + "] " + reportTrimmedRegions + " entire region(s) and " + reportTrimmedChunks + " individual chunk(s) trimmed so far (" + perc + "% done" + ")");
    }

    // send a message to the server console/log and possibly to an in-game player
    private void sendMessage(String text)
    {
        BentoBox.getInstance().log("[Trim] " + text);
